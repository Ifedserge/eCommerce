...
import { validateCity, validateDateOfBirth, validateEmail, validateLastName, validateName, validatePassword, validatePostalCode, validateStreet } from "../utilities/validation";
...
export function onDefaultShippingChange(event: Event) {
    const target = event.target as HTMLInputElement;
    const radios = document.querySelectorAll('input[name="defaultShipping"]') as NodeListOf<HTMLInputElement>;
    radios.forEach((radio) => {
        if (radio !== target) {
            radio.checked = false;
        }
...
export function onDefaultBillingChange(event: Event) {
    const target = event.target as HTMLInputElement;
    const radios = document.querySelectorAll('input[name="defaultBilling"]') as NodeListOf<HTMLInputElement>;
    radios.forEach((radio) => {
        if (radio !== target) {
            radio.checked = false;
        }
...
export function onSubmitUpdateUserButtonClick(event: Event) {
    ...
    if (!password || !newUser.email || !newUser.firstName || !newUser.lastName || !newUser.dateOfBirth) {
...
export function onSubmitUpdateAddressButtonClick(event: Event) {
    ...
    addresses.forEach((addressContainer) => {
        const address: IAddress = {
            ...
            streetName: (addressContainer.querySelector('input[name="streetName"]') as HTMLInputElement).value || '',
            streetNumber: (addressContainer.querySelector('input[name="streetNumber"]') as HTMLInputElement).value || '',
            city: (addressContainer.querySelector('input[name="city"]') as HTMLInputElement).value || '',
            postalCode: (addressContainer.querySelector('input[name="postalCode"]') as HTMLInputElement).value || '',
            country: (addressContainer.querySelector('select[name="country"]') as HTMLSelectElement).value || '',
        };
        ...
        if ((addressContainer.querySelector('input[name="defaultBilling"]') as HTMLInputElement).checked) {
            newUserAddress.defaultBillingAddress = address;
        }
        if ((addressContainer.querySelector('input[name="defaultShipping"]') as HTMLInputElement).checked) {
            newUserAddress.defaultShippingAddress = address;
        }

        if (!address.streetName || !address.streetNumber || !address.city || !address.postalCode || !address.country) {
            NotificationService.showNotification('Please enter all required fields', NotificationType.error);
            return;
        }
        ...
        if (!validateStreet(address.streetName).isValid) {
            NotificationService.showNotification('Please enter a valid street name', NotificationType.error);
