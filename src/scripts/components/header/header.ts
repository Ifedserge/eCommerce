...
export class Header {
  ...
  getLayout(): HTMLElement {
    ...
    wrapper.append(this.getLogo(), this.getManageBlock());
    return wrapper;
  }
  ...
  getLogo(): HTMLElement {
    ...
    `;
    const logoLink = createLink(['header__icon-wrapper'], '', ''); //добавить ссылку
    logoLink.innerHTML = logo;
    wrapper.append(logoLink);
    return wrapper;
  }

  getAuthBlock(): HTMLElement {
    //this.changeAuthState(); //эмулятор зарегистрированного или нет пользователя для view. Потом удалить

    const wrapper = createBlock(BlockType.div, ['header__auth-wrapper']);
    const registerLink = createLink(['header__link', 'text', 'text_normal'], '', 'Register'); //добавить ссылку
    const loginLink = createLink(['header__link', 'text', 'text_normal'], '', 'Log in'); //добавить ссылку
    const logoutLink = createLink(['header__link', 'text', 'text_normal'], '', 'Log out'); //добавить ссылку
    const userLink = createLink(['header__icon-wrapper'], '', ''); //добавить ссылку
  ...
  getManageBlock(): HTMLElement {
    ...
    const basketLink = createLink(['header__icon-wrapper'], '', ''); //добавить ссылку
    basketLink.innerHTML = basket;
    wrapper.append(this.getAuthBlock(), basketLink);
    return wrapper;
  }
